...
--2c **Challenge Question:** Are there any specialties that appear in the prescriber table that have no associated 
...
--(SELECT SUM(total_claim_count) AS total_sum
...
--SELECT DISTINCT(prescriber.specialty_description), SUM(prescription.total_claim_count) AS claim_count,
	CASE WHEN (SELECT opioid_drug_flag
				FROM drug
				WHERE opioid_drug_flag = 'Y')
...
SELECT generic_name, SUM(prescription.total_drug_cost)::money AS total_drug_cost
...
--alt logic
SELECT drug_name, drug.generic_name, SUM(prescription.total_drug_cost)::money AS total_cost 
FROM prescription
	FULL JOIN drug USING (drug_name)
GROUP BY drug_name, drug.generic_name
...
SELECT generic_name, SUM(prescription.total_drug_cost/total_day_supply)::money AS cost_per_day
FROM drug FULL JOIN prescription USING (drug_name)
GROUP BY generic_name
...
SELECT prescription.drug_name, drug.generic_name, (prescription.total_drug_cost)::money AS MONEY,
